...
if (!$error) {
	for ($i = 0 ; $i < $n ; $i++) {
		...
		$curdate = time();
		$sql = "INSERT INTO defect_recording_log(user_id,project_id,date,defect_no,type,inject,remove,fix_time,fix_defect,description) VALUES ('" . $_GET['user_id'] . "','" . $_GET['project_id'] . "','" . $_POST['date'][$i] . "','" . $_POST['defect_no'][$i] . "','" . $_POST['type'][$i] . "','" . $_POST['inject'][$i] . "','" . $_POST['remove'][$i] . "','" . $_POST['fixtime'][$i] . "','" . $_POST['fixdefect'][$i] . "','" . $_POST['description'][$i] . "')";
		$result = mysql_query($sql) or die(mysql_error());
	}
